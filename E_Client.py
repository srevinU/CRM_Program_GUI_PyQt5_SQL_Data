...
class EmailClient(QWidget):
    ...
    """THE EmailClient APPLICATION WINDOW.

            DISPLAY A WINDOW TO SEND AN EMAIL CAMPAIGN TO CLIENT FROM THE SQL DATA FILE data_clients.
    ...
    def __init__(self, Crm):
        ...
        """SET THE EmailClient WINDOW AND Crm AS THE PARENT"""

        self.Crm = Crm

        self.setWindowTitle("E-MAILING CLIENT")
    ...
    def home(self):
        ...
        """SET ALL THE WIDGETS OF THE EmailClient WINDOW"""

        label_client_address_country = QLabel("Country/Region:", self)
        self.grid.addWidget(label_client_address_country, 0, 0)
        self.box_client_address_country = QLineEdit(self)
        self.box_client_address_country.setClearButtonEnabled(True)
        self.grid.addWidget(self.box_client_address_country, 0, 1)

        label_client_address_province = QLabel("Province/Territory:", self)
        self.grid.addWidget(label_client_address_province, 1, 0)
        self.box_client_address_province = QLineEdit(self)
        self.box_client_address_province.setClearButtonEnabled(True)
        self.grid.addWidget(self.box_client_address_province, 1, 1)
    ...
    def btn_send_on(self):
        ...
        country = self.box_client_address_country.text()
        province = self.box_client_address_province.text()
    ...
    def auto_email(self):
        ...
        with conn:
            ...
            for i in c.execute("SELECT e_mail, country, province, news_letter FROM data_clients"):

                if (i[1] == self.box_client_address_country.text().upper() and
                    i[2] == self.box_client_address_province.text().upper() and
                    i[3] == "True") or (i[1] == self.box_client_address_country.text().upper() and
                        self.box_client_address_province.text().upper() == "ALL" and i[3] == "True"):
            ...
            add_mailing_data(self.date, "CLIENT", self.box_client_address_country.text().upper(),
                             self.box_client_address_province.text().upper(), self.e_mail_header.upper(),
                             self.count_email)

            self.Crm.message_box.append(f"{self.count_email} emails have been send to clients located "
                                        f"in the country {self.box_client_address_country.text().upper()} and "
                                        f"the province {self.box_client_address_province.text().upper()}")

            nb = nb_actions_data()

            add_actions_data(nb, "CLIENT EMAILING(" + str(self.count_email) + ")", self.date,
